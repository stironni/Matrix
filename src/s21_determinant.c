...
int s21_determinant(matrix_t *A, double *result) {
    ...
    *result = 0.0;
    ...
    if (A->rows == A->columns && A->matrix != NULL && (A->rows && A->columns) > 0) {
        ...
        if (A->rows == 2) *result = A->matrix[0][0]*A->matrix[1][1]-A->matrix[0][1]*A->matrix[1][0];
        if (A->rows > 2) {
            matrix_t cube;
            s21_create_matrix(A->rows - 1, A->rows - 1, &cube);
            for (int i = 0; i < A->columns; i++) {
                for (int j = 0; j < A->columns; j++) {
                    ...
                    s21_determinant(&cube, &result2);
                    *result += A->matrix[i][0] * pow(-1, i + j + 2) * result2;
                }
            }
            s21_remove_matrix(&cube);
        }
    } else {
        res = 1;
    }
