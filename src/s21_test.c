...
void s21_matrix_add(matrix_t *A) {
A->matrix[0][0] = 5;
A->matrix[0][1] = 2;
A->matrix[0][2] = 7;
A->matrix[0][3] = 5;
...
matrix_t result;
ck_assert_int_eq(s21_create_matrix(0, 50, &result), 1);
}
END_TEST
...
matrix_t result;
ck_assert_int_eq(s21_create_matrix(5, 0, &result), 1);
}
END_TEST
...
ck_assert_double_eq(s21_eq_matrix(&A, &B), 1);
s21_remove_matrix(&A);
s21_remove_matrix(&B);
}
END_TEST
...
matrix_t A, B;
s21_create_matrix(rows, columns, &A);
s21_create_matrix(5, columns, &B);
ck_assert_double_eq(s21_eq_matrix(&A, &B), 0);
s21_remove_matrix(&A);
s21_remove_matrix(&B);
}
END_TEST
